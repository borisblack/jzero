...
public:
  ...
  SymTabEntry(const char *sym, bool isConst, SymTab *parent_st, SymTab *st, std::shared_ptr<TypeInfo> typ, std::shared_ptr<Address> addr);
...
public:
  SymTabScope scope;
  SymTab *parent;
  std::map<std::string, SymTabEntry*> t;
  uint count;
  SymTab(SymTabScope scope) : SymTab(scope, NULL) {}
  SymTab(SymTabScope scope, SymTab *parent);
  ~SymTab();
  void insertEntry(const char *sym, bool isConst, SymTab *sub, std::shared_ptr<TypeInfo> typ);
  SymTabEntry *lookupEntry(const char *sym);
  size_t size();
  std::shared_ptr<ClassType> mkCls(char *name);
