...
void symtab_InsertEntry(SymTab *symTab, char *sym, bool isConst, SymTab *sub, TypeInfo *typ) {
  ...
  if (existingEntryNode) {
    ...
    newEntry.addr = tac_CreateAddress(scope(symTab->scope), symTab->count);
...
Address *symtab_Genlocal(SymTab *symTab) {
    char *s = malloc(25);
    sprintf(s, "__local$%u", symTab->count);
    symtab_InsertEntry(symTab, s, false, NULL, (TypeInfo*)typeinfo_IntType());
    return symtab_LookupEntry(symTab, s)->addr;
}
...
static void symtab_freeFunc(void* mem, void *arg) {
  ...
  typeinfo_Free(entryNode->typ);
  tac_FreeAddress(entryNode->addr);
